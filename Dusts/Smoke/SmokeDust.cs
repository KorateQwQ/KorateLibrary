...
public class SmokeDust : KLBasicDust
{
    ...
    {
        //ShouldBloom = true;
        noise ??= Mod.Assets.Request<Texture2D>("Effects/Tex/Noise/6", AssetRequestMode.ImmediateLoad).Value;
        base.OnSpawn(dust);
    }
    ...
    public override bool PreDraw(Dust dust)
    {
        ...
        maskScale: new Vector2(1f)); //,(Main.GameUpdateCount%60)/60f

        DrawInWorld(new TextureInfo(MainTexture, Frame, 2,2), dust.position, dust.GetRealColor(dust.color)*length,
            dust.Size() * new Vector2(1f), dust.rotation + 3.14f / 2f);
