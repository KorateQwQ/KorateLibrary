...
public class FireDust3 : KLBasicDust
{
    ...
    {
        int Frame = 0;
        Frame = (int)MathHelper.Lerp(0, 9, dust.LifeProgress());
        ...
        {
            se = (dust.DustInfo().DustData as int?) > 0 ? SpriteEffects.FlipVertically : SpriteEffects.None;
        }
        
        DrawInWorld(new TextureInfo(MainTexture,Frame,3,3), dust.position, dust.color,
            dust.Size(), dust.rotation,spriteEffects:se);  
